...
import usePaymentsProviders from "../hooks/usePaymentsProviders";



export default function PaymentsProvidersGrid(props) {
  const {
    filterDates = {
  ...
  useEffect(() => {
    const fetch03 = async () => {
      const data = await paymentsProviders.findAllBetweenDates(
        filterDates.start,
        filterDates.end
      );
    ...
    const fetch01 = async () => {
      const data = await paymentsProviders.findAllBetweenDatesPositiveBalance(
        filterDates.start,
        filterDates.end
      );
      const formatedData = data.map((item) => formatData(item));
    ...
    const fetch02 = async () => {
      const data = await paymentsProviders.findAllBetweenDatesZeroBalance(
        filterDates.start,
        filterDates.end
      );
      const formatedData = data.map((item) => formatData(item));
